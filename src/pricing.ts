...
export async function calculateCost(model: string, tokens: TokenCounts): Promise<number> {
	...
	// Reasoning tokens are billed as output tokens by every provider that
	// reports them (OpenAI's output_tokens includes them upstream — the Codex
	// loader carves them out into `reasoning`; Gemini bills `thoughts` at the
	// output rate; OpenCode's reasoning bucket likewise). Pricing them together
	// with output restores the exact pre-split Codex cost and fixes the silent
	// undercount for Gemini/OpenCode, whose reasoning was never priced before.
